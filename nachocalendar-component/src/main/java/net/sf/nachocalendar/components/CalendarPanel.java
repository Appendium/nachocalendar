...
public class CalendarPanel extends JPanel implements ChangeListener {
     ...
     */
    public CalendarPanel(final int quantity) {
     ...
     */
    public CalendarPanel(final boolean showWeekNumbers) {
     ...
     */
    public CalendarPanel(final int quantity, final int orientation) {
     ...
     */
    public CalendarPanel(int quantity, final int orientation, final boolean showWeekNumber, final boolean eternalScroll) {
        if (quantity < 1) {
            quantity = 1;
        }
        if (quantity > 12) {
    ...
    private void initScroll() {
        final int or = (orientation == HORIZONTAL ? Adjustable.HORIZONTAL : Adjustable.VERTICAL);
    ...
    public void setOrientation(final int orientation) {
        ...
        final int old = this.orientation;
        this.orientation = orientation;
        removeAll();
        if (orientation == VERTICAL) {
            layoutVertical();
    ...
    private void layoutHorizontal() {
        ...
        final JPanel centro = new JPanel(new GridLayout(1, months.length));
        for (int i = 0; i < months.length; i++) {
            centro.add(months[i]);
        }
        add(centro, BorderLayout.CENTER);
     ...
     */
    private void setShowingYear(final int year) {
        showingyear = year;
        for (int i = 0; i < months.length; i++) {
            cal.setTime(months[i].getMonth());
    ...
    public void setFirstDayOfWeek(final int firstDayOfWeek) {
        if ((firstDayOfWeek == Calendar.SUNDAY) || (firstDayOfWeek == Calendar.MONDAY)) {
            final int old = months[0].getFirstDayOfWeek();
    ...
    public void setScrollPosition(final int scrollPosition) {
        ...
        final int old = this.scrollPosition;
     ...
     */
    public void setYearPosition(final int yearPosition) {
        if (yearPosition == this.yearPosition) {
            return;
        }
        final int old = this.yearPosition;
    ...
    private boolean isShowing(final Date date) {
        if (date == null) {
            return false;
        }
        if (minDate == null) {
    ...
    private void showMonth(final Date d) {
        if (d == null) {
            return;
        }
     ...
     */
    public void setValue(final Object date) {
        try {
     ...
     */
    public void setValues(final Object[] dates) {
     ...
     * @param number
     */
    public void setMinimalDaysInFirstWeek(final int number) {
     ...
     */
    public void setEternalScroll(final boolean eternalScroll) {
        this.eternalScroll = eternalScroll;
    }
     ...
     */
    public void setShowToday(final boolean showToday) {
    ...
    public void dispose() {
        ...
        if (months != null) {
            for (int i = 0; i < months.length; i++) {
                final DayPanel[] daypanels = months[i].getDaypanels();
                for (int j = 0; j < daypanels.length; j++) {
                    daypanels[j].removeKeyListener(klistener);
                    daypanels[j].removeMouseListener(mlistener);
                    daypanels[j].dispose();
                    daypanels[j] = null;
                }
                months[i].dispose();
