...
public class MonthPanel extends JComponent {
    ...
    private int tcols;
    final int firstday;
    private int showingmonth, showingyear;
    private final Calendar calendar, check, navigation;
    ...
    private final HeaderPanel[] headers;
     ...
     */
    public MonthPanel(final boolean showWeekNumber) {
     ...
     * @param d new Date
     */
    public void setDay(final Date d) {
        setDay(d, true);
    }
     ...
     */
    public void setMonth(final Date d) {
        setDay(d, false);
    }
    ...
    private void setDay(final Date d, final boolean select) {
        if (d == null) {
            return;
        }
        date = d;
        final boolean update = ((showingyear != calendar.get(Calendar.YEAR)) || (showingmonth != calendar.get(Calendar.MONTH)));
        calendar.setTime(d);
        ...
        for (int i = 0; i < daypanels.length; i++) {
            if (update) {
                final Date temp = calendar.getTime();
     ...
     * @param e event fired
     */
    protected void fireChangeEvent(final ChangeEvent e) {
        final Iterator it = changelisteners.iterator();
        while (it.hasNext()) {
     ...
     */
    protected void setDaypanels(final DayPanel[] daypanels) {
