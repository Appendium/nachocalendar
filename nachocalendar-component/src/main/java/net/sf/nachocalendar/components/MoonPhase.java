...
public class MoonPhase extends Applet {
    ...
    String fixString(final double d) {
...
class MoonCanvas extends Canvas {
    ...
    private static final long serialVersionUID = 1L;
    private Dimension d;
    private final MoonPhase top;
    private int xmid, ymid;
    private final double scale;

    public MoonCanvas(final MoonPhase parent) {
        top = parent;
    ...
    protected void paintSun(final Graphics g) {
        final int size = (int) (d.width * scale * top.sunsize / 2);

        g.setColor(Color.yellow);
        g.fillOval(xmid - size - (int) (d.width * scale * top.sundist), ymid - size, size * 2, size * 2);
    }

    protected void paintMoon(final Graphics g) {
        final int size = (int) (d.width * scale * top.moonsize / 2);

        g.setColor(Color.darkGray);
        g.fillOval(xmid - size + (int) (d.width * scale * top.moondist * Math.cos(top.phase * 2.0 * Math.PI)), ymid - size
                - (int) (d.width * scale * top.moondist * Math.sin(top.phase * 2.0 * Math.PI)), size * 2, size * 2);
        g.setColor(Color.white);
        g.fillArc(xmid - size + (int) (d.width * scale * top.moondist * Math.cos(top.phase * 2.0 * Math.PI)), ymid - size
                - (int) (d.width * scale * top.moondist * Math.sin(top.phase * 2.0 * Math.PI)), size * 2, size * 2, 90, 180);
    }

    protected void paintEarth(final Graphics g) {
        final int size = (int) (d.width * scale * top.earthsize / 2);
    ...
    protected void paintSky(final Graphics g) {
        d = getSize();
        xmid = d.width / 2;
        ymid = d.height / 2;
...
class MoonCanvas2 extends Canvas {
    ...
    private final MoonPhase top;
    private int xmid, ymid;
    private final double scale, extrasun;

    public MoonCanvas2(final MoonPhase parent) {
        top = parent;
        scale = 0.2;
        extrasun = 1.05;
    ...
    protected void paintSun(final Graphics g) {
        final int size = (int) (d.width * scale * extrasun / 2);

        g.setColor(Color.yellow);
        g.fillOval(xmid - size - (int) (d.width * scale * (top.phase - 0.5) * 720), ymid - size, size * 2, size * 2);
    }

    protected void paintMoon(final Graphics g) {
        int i, limit, sign;
        final int size = (int) (d.width * scale / 2);
        double p = top.phase;
        ...
        for (i = -limit; i <= limit; i++) {
            g.drawLine(xmid + sign * (int) (limit * Math.sqrt(1.0 - (double) (i * i) / ((double) (limit * limit)))), ymid + i, xmid + sign
                    * (int) (limit * Math.cos(2.0 * Math.PI * p) * Math.sqrt(1.0 - (double) (i * i) / ((double) (limit * limit)))), ymid + i);
        }

    }

    public void paintSky(final Graphics g) {
    ...
    public void paintSky(final Graphics g, final int x, final int y, final int xm, final int ym) {
        // pre-clear the bitmap or the applet
        // remove this if you paint the entire area anyway
...
class PhaseThread implements Runnable {
    ...
    public PhaseThread(final MoonPhase parent) {
        super();
    }
