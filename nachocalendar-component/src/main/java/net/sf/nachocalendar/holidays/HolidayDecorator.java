...
public class HolidayDecorator implements DayRenderer {
    private final DayRenderer renderer;

    /**
     * Default constructor.
     * @param renderer renderer to decore.
     */
    public HolidayDecorator(final DayRenderer renderer) {
        super();
    ...
    public Component getDayRenderer(final DayPanel daypanel, final Date day, final Object data, final boolean selected, final boolean working,
        ...
        final Component retorno = renderer.getDayRenderer(daypanel, day, data, selected, working, enabled);
        if (!enabled) {
            return retorno;
        }
        if (data != null) {
